...
namespace MultiThread_vs_Single
{
    partial class Program
    {
        ...
        private static void Create_Objects_Method_Multithread(int i)
        {
            DateTime A = DateTime.Now;
            ...
            Console.WriteLine("\tTest Thread ID: {0}.\t Time: {1} ms.", Thread.CurrentThread.ManagedThreadId, C.TotalMilliseconds);
        }
        ...
        private static void Chreate_New_Threads()
        {
            ...
            for (int i = 0; i < Repeats; i++)
            {
                Thread_List.Add(new Thread(() => Create_Objects_Method_Multithread(i + 1)));
            }
